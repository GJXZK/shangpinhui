<template>
  <div class="register-container">
    ...
    <div class="register">
      ...
      <div class="content">
        <label>确认密码:</label>
        <input type="text" placeholder="请输入确认密码" v-model="passwordAgain">
        <span class="error-msg">错误提示信息</span>
      </div>
      <div class="controls">
...
<script>
  export default {
    ...
    methods:{
      async getCode(){
        let code=await this.$store.dispatch('user/getCode',this.phone)
        this.code=code
      },
      async register(){
        try {
          const {phone,code,password,passwordAgain}=this;
          (phone&&code&&password==passwordAgain)&& await this.$store.dispatch('user/register',{phone,code,password})
